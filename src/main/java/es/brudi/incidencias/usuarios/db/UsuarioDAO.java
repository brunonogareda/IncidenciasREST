...
public class UsuarioDAO {
	 ...
	 */
	protected static Usuario comprobarUsuario(String username, String password) {
		...
		try (PreparedStatement pst = conn.prepareStatement(query)) {
			...
			if(res.next()) {
				...
				if(username.equals(user) && passwordHash.equalsIgnoreCase(pass)) {
					
					ArrayList<Integer> instalacions = UsuarioDAO.getInstalacionsXestionadas(id);
					Cliente c = ClienteAccessor.getClienteById(idCliente);
					Grupo g = GrupoAccessor.getGrupoById(idGrupo);
					instalacions.addAll(g.getInstalacionsXestionadas());
